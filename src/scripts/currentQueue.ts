...
$(() => {
	...
	$('#currentQueue .currentSong .more').on('click', (event) => {
		let elem = $(event.target)
		let pos = {x: event.clientX, y: event.clientY}
		showTooltipForSong(musicPlayer.currentSong, {pos: pos})
	})

	$('#currentQueue .currentSong .like').on('click', async (event) => {
...
function updateSongFocus() {
	...
	if(song.liked) $('#currentQueue .currentSong .like').attr('src', './images/red-heart.png')
